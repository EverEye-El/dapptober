  ...
  return (
    ...
    >
      ...
      <div className="relative h-48 overflow-hidden bg-gradient-to-br from-primary/20 via-accent/20 to-primary/10">
        <Image
          src={submission.image_url || `/placeholder.svg?height=192&width=384&query=dapp day ${submission.dapp_day}`}
      ...
      <div className="p-4 space-y-3">
        <div className="space-y-2">
          <h3 className="text-lg font-bold text-balance leading-tight gradient-text">{submission.title}</h3>
        ...
        <div className="flex items-center gap-2 pt-2">
          <Avatar className="w-6 h-6 border border-primary/30">
            <AvatarImage
              src={submission.profile.avatar_url || "/placeholder.svg"}
              alt={submission.profile.display_name || "User"}
            />
            <AvatarFallback className="text-xs bg-primary/20">
              {submission.profile.display_name?.[0]?.toUpperCase() || "U"}
            </AvatarFallback>
          </Avatar>
          <div className="flex-1 min-w-0">
            <p className="text-xs font-medium text-foreground truncate">
              {submission.profile.display_name || truncateAddress(submission.profile.wallet_address)}
            </p>
